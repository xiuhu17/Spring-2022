...
\subsection*{5.1.10}
\begin{proof}
    Since $T$ is invertible, so $n = dimV = dimW$. Since $\forall i \in \{1, 2, ..., n\}, \sigma_i > 0, \ne 0$. By \textbf{Theorem5.3}, we know the singular value is unique.
    \begin{denote}
    ...
    \begin{equation*}
        \begin{split}
            \|T(v)\| & = \|T(\sum_{i = 1}^{i = n} a_ie_i)\| = \|\sum_{i = 1}^{i = n} a_iT(e_i)\| = \|\sum_{i = 1}^{i = n} a_i\sigma_if_i\| \\
                & \ge \| \sigma_n\sum_{i = 1}^{i = n} a_if_i\| = |\sigma_n| \| \sum_{i = 1}^{i = n} a_if_i\| \\
                & = |\sigma_n|\sqrt{\sum_{i = 1}^{i = n} \|a_if_i\|^2} = |\sigma_n|\sqrt{\sum_{i = 1}^{i = n} |a_i|^2\|f_i\|^2} \\
